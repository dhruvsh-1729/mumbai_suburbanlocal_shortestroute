...
const stations = //All stations with their respective adjacent stations
    [
        ...
        {
            "name": "Kopar",
            "adjacentStations": ["Diva Junction", "Dombivli"],
            "x": 7,
            "y": 24
        },
        {
...
app.post('/api/station', (req, res) => {
    try {
        ...
        const filteredStations = stations.filter(station => station.name.toLowerCase().startsWith(inputValue.toLowerCase()))

        const result = filteredStations.map(station => { return { value: station.name, label: station.name } })
...
app.post('/api/shortestpath',(req, res) => {
    try {
        ...
        function buildGraph(stations) {
          ...
          const shortestPath = findShortestPath(stations, src, dest);

        res.status(200).json({ message: shortestPath, success: true })
    }
